...
Шаг 1 — PostgreSQL:
  Фильтрация лекций по семестру и требованиям к оборудованию.
...
# Neo4j — графовая СУБД; GraphDatabase — драйвер для подключения и выполнения Cypher-запросов
from neo4j import GraphDatabase
# psycopg2 — драйвер PostgreSQL; реляционная СУБД — основной источник данных (лекции, расписание, студенты)
import psycopg2
# redis — клиент Redis; кэш-хранилище данных студентов (Hash student:{id}, TTL 2 ч)
import redis
# pymongo — драйвер MongoDB; документная СУБД для иерархии университета (вложенный документ вместо 4 JOIN)
from pymongo import MongoClient
# jwt — библиотека PyJWT; декодирование и проверка JWT-токенов сервисной аутентификации
import jwt
import os
import logging
import time
...
# Секретный ключ и алгоритм для проверки JWT-токенов.
# Логика проверки: jwt.decode() проверяет подпись (HMAC-SHA256) и срок действия (exp);
# затем дополнительно проверяется поле type == "service" — принимаются только сервисные токены,
# пользовательские токены (type=user) отклоняются с 403.
JWT_SECRET = os.environ.get("JWT_SECRET", "polyglot_jwt_secret_key_2026")
JWT_ALGORITHM = os.environ.get("JWT_ALGORITHM", "HS256")
security = HTTPBearer()


# Защита «эшелонированная» (defense in depth): принимаются ТОЛЬКО сервисные токены (type=service).
# Пользовательские токены не проходят проверку, даже если подпись верна — это предотвращает
# доступ конечных пользователей к внутренним сервисам напрямую.
...
# Параметры подключения к четырём БД (PostgreSQL, Neo4j, Redis, MongoDB),
# читаются из переменных окружения Docker-контейнеров.
...
# PostgreSQL — реляционная СУБД: хранит лекции, курсы, расписание, группы, студентов.
# Используется на шагах 1 и 3 (fallback при промахе кэша Redis).
def get_pg():
    return psycopg2.connect(host=PG_HOST, port=PG_PORT, dbname=PG_DB, user=PG_USER, password=PG_PASS)


# Neo4j — графовая СУБД: хранит связи Lecture→Course, Lecture←Schedule←Group.
# Используется на шаге 2 для обхода графа и сужения множества групп.
def get_neo4j():
    return GraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USER, NEO4J_PASS))


# Redis — кэш-хранилище: хранит данные студентов как Hash student:{id} с TTL 2 ч.
# Используется на шаге 3 для быстрого получения данных студентов (cache-aside).
def get_redis():
    return redis.Redis(host=REDIS_HOST, port=REDIS_PORT, decode_responses=True)


# MongoDB — документная СУБД: хранит иерархию университета как вложенный документ
# (University→Institutes→Departments→Specialities). Используется на шаге 4.
def get_mongo():
    return MongoClient(host=MONGO_HOST, port=MONGO_PORT, username=MONGO_USER, password=MONGO_PASS)


# Основной запрос: 4-шаговый конвейер PostgreSQL → Neo4j → Redis → MongoDB
# Шаг 1 (PG):  фильтрация лекций по семестру/оборудованию, расписание по году, COUNT студентов
# Шаг 2 (Neo4j): обход графа для получения связей Lecture→Course и Lecture←Schedule←Group
# Шаг 3 (Redis): данные студентов только для групп из Neo4j (не всех!), cache-aside с fallback на PG
# Шаг 4 (Mongo): findOne иерархии университета — вложенный документ вместо 4 JOIN в PG
@app.get("/query")
def query_schedule_capacity(
    semester: int = Query(..., description="Номер семестра (1-8)"),
    year: int = Query(..., description="Год обучения"),
    equipment: str = Query("", description="Требования к оборудованию"),
    ...
    # ===== ШАГ 1: PostgreSQL =====
    # 1a) Batch-фильтрация лекций: только лекции типа «лекция» для заданного семестра;
    #     если указано оборудование — дополнительный фильтр по equipment_req ILIKE или tags.
    ...
    query = """
        SELECT l.id, l.title, l.lecture_type, l.equipment_req, l.tags,
    ...
    if equipment:
        # Фильтр по требованиям к оборудованию: поиск подстроки в equipment_req ИЛИ точное совпадение в массиве tags
        query += " AND (l.equipment_req ILIKE %s OR %s = ANY(l.tags))"
        params.extend([f"%{equipment}%", equipment])

    cur.execute(query, params)
    lecture_rows = cur.fetchall()

    # 1b) Собираем словари: lecture_info (данные лекции) и course_info (данные курса + speciality_id)
    ...
    # 1c) Batch-запрос расписания: только записи за указанный год (BETWEEN year_start AND year_end)
    #     для лекций из шага 1a. Собираем schedule_by_lecture и множество all_group_ids.
    ...
    # 1d) COUNT студентов в каждой группе: GROUP BY + COUNT — определяем численность групп
    #     для расчёта требуемой вместимости аудитории
    ...
    steps.append({
        "step": 1,
        "store": "PostgreSQL",
        "action": f"Batch: lecture filter (semester={semester}, equipment='{equipment}'), schedule by year={year}, GROUP BY student count — O(N), composite index (lecture_id, week_start_date)",
    ...
    # ===== ШАГ 2: Neo4j =====
    # Обход графа: Lecture-[BELONGS_TO]->Course — привязка лекции к курсу;
    #              Lecture<-[PART_OF]-Schedule<-[CONTAINS]-Group — расписание и группы лекции.
    # Результат: сужение множества групп (не все из PG, а только те, что связаны через расписание)
    # для уменьшения числа запросов к Redis на шаге 3.
    ...
    with driver.session() as session:
        # Cypher-запрос: начиная с лекций из шага 1, обходим рёбра BELONGS_TO, PART_OF, CONTAINS
        # и получаем уникальные привязки lecture_id→course_id, lecture_id→group_id через schedule
        result = session.run("""
            MATCH (l:Lecture)
    ...
    # ===== ШАГ 3: Redis =====
    # Запрашиваем данные студентов ТОЛЬКО для групп из Neo4j (не всех из PG!).
    # Это ключевая оптимизация: Neo4j сузил множество групп до тех, что реально
    # привязаны к расписанию лекций, поэтому в Redis идёт меньше запросов.
    # Стратегия cache-aside: сначала pipeline HGETALL из Redis, при промахе — fallback к PG,
    # результат записывается в кэш с TTL 2 ч. Batch-размер pipeline = 2000 для баланса
    # пропускной способности и потребления памяти.
    neo_group_ids_list = list(neo_group_ids)

    logger.info(f"Step 3: Redis - pipeline for {len(neo_group_ids_list)} groups only")
    r = get_redis()

    # Сначала получаем из PG только id студентов, относящихся к Neo4j-группам
    pg = get_pg()
    cur = pg.cursor()
    ...
    # Cache-aside: pipeline HGETALL для каждого student:{id} батчами по 2000
    # Если Hash найден — попадание (cache_hits), иначе — промах (cache_misses)
    student_details = {}
    cache_hits = 0
    cache_misses = []
    ...
    # Fallback к PostgreSQL при промахах кэша: батчами по 5000
    # Записываем найденные данные обратно в Redis (HSET + EXPIRE 7200с = 2ч)
    if cache_misses:
        pg = get_pg()
        cur = pg.cursor()
    ...
    # Группируем студентов по group_id для итоговой сборки
    students_by_group = {}
    ...
    steps.append({
        "step": 3,
        "store": "Redis",
        "action": f"Pipeline HGETALL student:{{id}} (Hash, TTL=2ч) для {len(relevant_student_ids)} студентов из {len(neo_group_ids_list)} групп Neo4j (не всех!). Попаданий: {cache_hits}, промахов: {len(cache_misses)}",
        "result": f"Получены данные {len(student_details)} студентов (сужено Neo4j)"
    })

    # ===== ШАГ 4: MongoDB =====
    # Чтение вложенного документа иерархии университета: findOne — O(1) вместо 4 JOIN в PG
    # (University→Institutes→Departments→Specialities).
    # Документ хранит всю иерархию целиком, что позволяет обойти реляционные JOIN.
    ...
    # Разбираем вложенный документ: University→Institutes→Departments→Specialities
    # и строим словарь speciality_id → {университет, институт, кафедра, специальность, код}
    hierarchy_info = {}
    if hierarchy_doc:
    ...
    # ===== ФИНАЛЬНАЯ СБОРКА =====
    # Объединяем данные из всех 4 хранилищ:
    #   - course_info (PG) — данные курса
    #   - course_lecture_map (Neo4j) — привязки лекций, групп, расписаний к курсу
    #   - lecture_info + schedule_by_lecture (PG) — детали лекций и расписание
    #   - group_student_counts (PG) — численность групп для расчёта listeners
    #   - students_by_group (Redis/PG fallback) — данные студентов (первые 10 на группу)
    #   - hierarchy_info (MongoDB) — иерархия университета по speciality_id
    # Итог: для каждого курса — лекции с расписанием и числом слушателей,
    #        группы со студентами, требуемая вместимость аудитории = max listeners.
    final_results = []
        ...
        for lid in cdata["lecture_ids"]:
            ...
            for sch in scheds:
                lecture_details.append({
                    ...
                    "title": li.get("title", ""),
                    "type": li.get("type", ""),
                    "equipment_req": li.get("equipment_req", ""),
                    "tags": li.get("tags", []),
    ...
    return {
        ...
        "justification": {
            "PostgreSQL": "Batch-агрегация: schedule по году + COUNT студентов + фильтр оборудования — composite index (lecture_id, week_start_date)",
...
@app.get("/")
def root():  # health-check эндпоинт
